...
class RClip:
  ...
  def __init__(self, exclude_dirs: Optional[List[str]] = []):
    self._model = model.Model()
    datadir = utils.get_app_datadir()
    self._db  = db.DB(datadir / 'db.sqlite3')
  ...
  def search(
      ...
      positive_queries: List[str] = [], negative_queries: List[str] = [], skip_index: bool = False) -> List[SearchResult]:
    if not skip_index:
        self.ensure_index(directory)
